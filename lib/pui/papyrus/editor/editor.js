...
YAHOO.widget.papyrus.Editor = new function() { 
	...
	this.init = function() {
		...
		this.setEditorNotification = function( message ) { console.log( editorNotificationElId ); Dom.get( editorNotificationElId ).innerHTML = message; };
		...
		// Add label to breadcrumb
		this.appendLabelToBreadcrumb = function( label ) {
		...
		this.appendEditorToBreadCrumb = function( _id, _contentType ) {
			...
			editor = this.getEditor( _contentType );
			
			// Ensure editor exists
			if( editor != null ) {
				
				// Check here if link clicked on is already the editor loaded and prevent it from loading.
				...
				breadcrumbs[ breadcrumbs.length ] = { id : _id, contentType : _contentType, name : editor.getName() };
				breadcrumbHTMLEl = Dom.get( editorBreadcrumbElId );
				breadcrumbHTML = breadcrumbHTMLEl.innerHTML + '>&nbsp;&nbsp;<a href="#" onclick="YAHOO.widget.papyrus.Editor.switchToEditorInBreadCrumb( ' + ( breadcrumbs.length - 1 ) + ' );">' + editor.getName() + '</a>&nbsp;&nbsp;';
		...
		this.switchToEditorInBreadCrumb = function( index ) {
			...
			breadcrumbHTML = "";
			for( var i = 0; i < breadcrumbs.length; i++ ) {
				breadcrumbHTML += '>&nbsp;&nbsp;<a href="#" onclick="YAHOO.widget.papyrus.Editor.switchToEditorInBreadCrumb( ' + i + ' );">' + breadcrumbs[ i ].name + '</a>&nbsp;&nbsp;';
			}
			this.setEditorBreadcrumb( breadcrumbHTML );
	...
	this.initEvents = function() {
		...
		// Event delgation
		YAHOO.util.Event.addListener( document.body, "change", onInputChange, this );
